...
from calculators.metal.steel_weights.tee_bar import TeeBarInput, calculate_tee_bar

def render():
    st.header("Tee Bar Weight (T-Section)")

    st.subheader("Dimensions")
    cols = st.columns(5)
    with cols[0]:
        depth_mm = st.number_input("Depth h (mm)", min_value=0.0, value=100.0)
    with cols[1]:
        bf_mm = st.number_input("Flange width bf (mm)", min_value=0.0, value=100.0)
    with cols[2]:
        tf_mm = st.number_input("Flange thk tf (mm)", min_value=0.0, value=8.0)
    with cols[3]:
    ...
    if st.button("Calculate"):
        out = calculate_tee_bar(TeeBarInput(
