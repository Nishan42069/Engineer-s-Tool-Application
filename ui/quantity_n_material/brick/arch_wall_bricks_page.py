...
def render():
    ...
    with g_cols[3]:
        span = st.number_input("Arch span (ft)", min_value=0.0, value=5.0)

    arch_cols = st.columns(2)
    with arch_cols[0]:
        rise = st.number_input("Arch rise (ft)", min_value=0.0, value=2.0)
    with arch_cols[1]:
    ...
    if st.button("Calculate arch wall bricks"):
