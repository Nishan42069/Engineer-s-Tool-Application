...
from calculators.quantity_n_material.soil_mechanics.bc_circular import (
    BearingCapacityCircularInput,
    calculate_bearing_capacity_circular,
)


def render():
    st.header("Bearing Capacity – Circular Footing")
    ...
    with c4:
        B = st.number_input("Footing diameter B (m)", min_value=0.0, value=2.0)
    with c5:
    ...
    if st.button("Calculate bearing capacity (circular)"):
        inp = BearingCapacityCircularInput(
            cohesion_kPa=cohesion,
            phi_deg=phi,
            unit_weight_kN_m3=gamma,
            footing_diameter_m=B,
            depth_foundation_m=Df,
            factor_of_safety=fs,
        )
        out = calculate_bearing_capacity_circular(inp)
