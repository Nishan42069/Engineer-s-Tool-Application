...
from calculators.quantity_n_material.soil_mechanics.bc_continuous import (
    BearingCapacityContinuousInput,
    calculate_bearing_capacity_continuous,
)


def render():
    st.header("Bearing Capacity – Continuous (Strip) Footing")
    ...
    if st.button("Calculate bearing capacity (continuous)"):
        inp = BearingCapacityContinuousInput(
        ...
        out = calculate_bearing_capacity_continuous(inp)
