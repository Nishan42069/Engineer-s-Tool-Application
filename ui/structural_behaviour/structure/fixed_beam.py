...
from calculators.structural_behaviour.structure.fixed_beam import (
    FixedBeamInput,
    calculate_fixed_beam,
)

def render():
    st.header("Fixed Beam")
    ...
    if st.button("Calculate Fixed Beam"):
        inp = FixedBeamInput(length_ft=length_ft, load_value=load_value)
        out = calculate_fixed_beam(inp)
